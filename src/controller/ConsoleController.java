...
public class ConsoleController {
			...
			try {
				...
				switch (a) {
					...
					case 2 :
						...
						break;
					case 3 :
						this.insuranceMenu(insuranceProductService
								.showInsuranceProductIsApproval());
						break;
					case 4 :
	...
	private void managerRegisterMenu() { // 0611 변경
		while (true) {
			try {
				...
				System.out.println(
						"[1.보험상품개발자 2.보험상품승인자 3.U/W 4.계약관리자 5.보상처리자 6.영업사원]");
				int input = sc.nextInt();
				Manager manager = ManagerType.values()[input - 1].getManager()
						.clone();
				...
				while (managerService.checkManagerID(manager.getId()) != null) {// 중복된
																				// 아이디
																				// 체크
																				// 변경(21.05.31)
				...
				System.out.println(managerService.register(manager)
						? "등록이 완료되었습니다."
						: "등록에 실패하였습니다.");
	...
	private void managerDeleteMenu() {
		...
		System.out.println(managerService.delete(id, pw)
				? "삭제가 완료되었습니다."
				: "삭제에 실패하였습니다.");
				...
				switch (sc.nextInt()) {
					case 1 :
						...
						if (developInsuranceProduct != null) {
							developInsuranceProduct = developInsurance(
									developInsuranceProduct);
							if (developInsuranceProduct != null)
								insuranceProductService.addInsuranceProduct(
										developInsuranceProduct);
							else
	...
	private InsuranceProduct developInsurance(
	...
	private InsuranceProduct designInsurance() {// try catch문을 써서 5이상의 값과 문자를 넣을
												// 경우 예외처리 (21.05.24)
		System.out.println("\n개발할 보험을 선택해주세요.");
		...
		while (true) {
			try {
				...
				sc.nextLine();
				if (input <= 4) {
					InsuranceProduct developInsuranceProduct = InsuranceProductType
							.values()[input - 1].getInsuranceProduct().clone();
	...
	private ActualExpense developActualExpense(
			InsuranceProduct insuranceProduct) {// 0611 수정
		try {
			...
			while (insuranceProductService.searchInsuranceProduct(
					actualExpense.getProductName()) != null) {
			...
			int input = sc.nextInt();
			actualExpense.setActualExpenseType(
					ActualExpenseType.values()[input - 1]);
			System.out.println(ActualExpenseType.values()[input - 1]
					.getactualexpensename());
	...
	private Cancer developCancer(InsuranceProduct insuranceProduct) {// sysout 글
																		// 수정
																		// (21.05.24)
		try {
			...
			while (insuranceProductService
					.searchInsuranceProduct(cancer.getProductName()) != null) {
				System.out.println("이미 존재하는 제품명입니다. 다시 입력해주세요.");
				cancer.setProductName(sc.nextLine());
			}
			...
			System.out.println(
					"1.췌장암(1.6) 2.폐암(1.5) 3.위암(1.4) 4.대장암(1.3) 5.간암(1.2) 6.기타(1.1)");
			int input = sc.nextInt();
			if (input == 7)
				throw new IndexOutOfBoundsException();
			cancer.setGuaranteedType(CancerType.values()[input - 1]);// 암보험
																		// nullpointer해결(21.05.30)
			double rate = CancerType.values()[input - 1].getRate();
			System.out.println(CancerType.values()[input - 1].getCancerName()
					+ " " + rate);
	...
	private Pension developPension(InsuranceProduct insuranceProduct) {// sysout
																		// 글 수정
																		// (21.05.24)
		try {
			...
			while (insuranceProductService
					.searchInsuranceProduct(pension.getProductName()) != null) {
				System.out.println("이미 존재하는 제품명입니다. 다시 입력해주세요.");
				pension.setProductName(sc.nextLine());
			}
	...
	private Life developLife(InsuranceProduct insuranceProduct) {// sysout 글 수정
																	// (21.05.24)
		try {
			...
			while (insuranceProductService
					.searchInsuranceProduct(life.getProductName()) != null) {
				System.out.println("이미 존재하는 제품명입니다. 다시 입력해주세요.");
				life.setProductName(sc.nextLine());
			}
	...
	private void insuranceProductsAcceptanceMenu() {// 0611 수정
		// InsuranceProductsAcceptance ipa = (InsuranceProductsAcceptance)
		// managerLogin;
		while (true) {
			try {
				System.out.println("---InsuranceProductsAcceptanceMenu---");
				System.out.println(
						"1.승인할 보험 선택하기 2.승인된 보험 삭제 3.금융감독원에게 승인메일 보내기 4.로그아웃");
				int a = sc.nextInt();
				if (a < 1 || a >= 5) {
					System.out.println("올바른 값을 입력해주세요.\n");
				}
				switch (a) {
					case 1 :
						if (insuranceProductService
								.showInsuranceProductIsNotApproval()
								.isEmpty()) {
							System.out.println("현재 만들어진 보험이 없습니다.");
							break;
						} else
							approvalMenu(insuranceMenu(insuranceProductService
									.showInsuranceProductIsNotApproval()));
						break;
					case 2 :
			...
			try {
				...
				switch (input) {
					case 1 :
						...
						System.out.println(insuranceProductService
								.modifyInsuranceProduct(insuranceProduct)
										? "승인이 완료되었습니다."
										: "승인에 실패하였습니다.");
						return;
					case 2 :
						System.out.println(insuranceProductService
								.deleteInsuranceProduct(insuranceProduct)
										? "승인이 거절 되었습니다. 목록에서 삭제합니다."
										: "승인거절에 문제가 발생하였습니다.");
						return;
					case 3 :
						return;
	...
	private void approvalInsuranceDelete() {// 0611 변경
		while (true) {
			try {
				if (insuranceProductService.showInsuranceProductIsApproval()
						.isEmpty()) {
				...
				int i = 1;
				ArrayList<InsuranceProduct> list = insuranceProductService
						.showInsuranceProductIsApproval();
				for (InsuranceProduct insuranceProduct : list) {
					System.out.println(i + ".상품명: "
							+ insuranceProduct.getProductName() + " 보험종류: "
							+ insuranceProduct.getInsuranceProductType()
									.getInsuranceName());
					i++;
				}
				System.out.println("\n--삭제할 보험을 선택해주세요.--");
				...
				switch (y) {
					case 1 :
						System.out.println(insuranceProductService
								.deleteInsuranceProduct(deleteInsuranceProduct)
										? "삭제가 완료되었습니다."
										: "삭제에 실패하였습니다.");
						return;
					case 2 :
	...
	private void followUpInsurance() {// 사후관리 0611 수정
		...
		InsuranceProduct selectedInsuranceProduct = this.insuranceMenu(
				insuranceProductService.showInsuranceProductIsApproval());
		if (selectedInsuranceProduct == null) {// 준비된 상품이 없을 시 아래 sysout이 나오는거
												// 수정 (21.05.31)
			return;
		}
		System.out.println("\n해당 보험을 수정하시겠습니까? \n1.수정하기 2.뒤로가기");
	...
	private boolean modifyInsuranceProduct(
			InsuranceProduct selectedInsuranceProduct) { // 0611 변경
		switch (selectedInsuranceProduct.getInsuranceProductType()) {
			case ACTUALEXPENSE :
				return insuranceProductService.modifyInsuranceProduct(
	...
	private Contract selectUnderWriteContract() {
		ArrayList<Contract> contractList = contractService
				.selectByApproval(false);
		if (contractList.size() > 0) {
			System.out.println("[인수심사 계약 목록]");
			for (int i = 0; i < contractList.size(); i++)
				System.out.println((i + 1 + ". ")
						+ contractList.get(i).getClient().getName() + " "
						+ contractList.get(i).getInsuranceProduct()
								.getInsuranceProductType().getInsuranceName());
	...
	private void showClientInfo(Contract contract) { // 0611 변경
		...
		switch (contract.getInsuranceProduct().getInsuranceProductType()) {
			case ACTUALEXPENSE :
				System.out.println("입원내역: " + client.getMedicalHistory()
						.getNumberOfHospitalizations());
				System.out.println("병원진료: " + client.getMedicalHistory()
						.getNumberOfHospitalVisits());
				break;
			case CANCER :
				System.out
						.println("암경력: "
								+ client.getMedicalHistory()
										.getClientCancerCareer().getCancerName()
								+ "(본인)"
								+ client.getMedicalHistory()
										.getFamilyCancerCareer().getCancerName()
								+ "(가족)");
				break;
			case LIFE :
				break;
	...
	private void insuranceContract(ContractManagement contractManagement) { // 0611
																			// 수정
		while (true) {
			try {
				...
				ArrayList<Contract> contractList = contractService
						.selectByInsuranceProductType(
								InsuranceProductType.values()[a - 1]);
	...
	private void showContractList(ArrayList<Contract> contractList) {
		...
		for (Contract contract : contractList) {
			System.out.println("계약번호: " + i + " 고객이름: "
					+ contract.getClient().getName() + " 보험이름: "
					+ contract.getInsuranceProduct().getProductName()
					+ " 가입날짜: " + contract.getInsuranceContractDate()
					+ " 만기날짜: " + contract.getInsuranceExpiryDate() + " 승인여부: "
					+ contract.isApproval());
			i++;
		}
	}

	private void showContractInfo(Contract contract) {// 가입한 보험과 영업사원 이름 안나오는 현상
														// 해결(21.05.26)
		Client client = contract.getClient();
		...
		System.out.println("가입한 보험: " + insuranceProduct
				.getInsuranceProductType().getInsuranceName());
	...
	private void showExpiredInsuranceMenu() { // 0611 변경
		while (true) {
			try {
				...
				ArrayList<Contract> contractList = contractService
						.selectByExpiredDate(
								InsuranceProductType.values()[a - 1]);// 4를 입력했을
																		// 경우
																		// 인덱스
																		// 오류
																		// 해결.(21.05.31)
				...
				if (!contractList.isEmpty()) {
					System.out.println("상세정보를 확인하실 계약번호를 입력하세요.");
					showExpiredInsuranceInfo(
							contractList.get(sc.nextInt() - 1));
				} else {
					System.out.println("계약이 존재하지 않습니다.");
				}
			} catch (InputMismatchException e) {
	...
	private void accidentHandlingMenu(CompensationHandle compensationHandle) {// 0611
																				// 변경
		while (true) {
			try {
				...
				if (!accidentList.isEmpty()) {
					for (Accident accident : accidentList) {
						...
						System.out.println((i + 1) + ". " + "고객이름: "
								+ client.getName() + " 상품명: "
								+ accident.getInsuranceProduct()
										.getProductName()
								+ " 접수날짜: "
					...
					input = sc.nextInt();
					this.showAccidentDetail(compensationHandle,
							accidentList.get(input - 1));
				} else {
	...
	private void showAccidentDetail(CompensationHandle compensationHandle,
			Accident accident) { // 0611 변경
		try {
			Client client = accident.getClient();
			...
			switch (sc.nextInt()) {
				case 1 :
					...
					System.out.println(compensationHandle
							.payInsuranceMoney(sc.nextInt(), client)
									? "보험금 지급이 완료되었습니다."
									: "보험금 지급에 실패하였습니다.");
					System.out.println(
							contractService.deleteAccidentList(accident)
									? "사고기록 삭제에 성공하였습니다."
									: "사고기록 삭제에 실패하였습니다.");
					break;
				case 2 :
	...
	private void salesPersonMenu() {// SP(영업사원) //EA바뀜 0611 수정
		while (true) {
			try {
				...
				switch (sc.nextInt()) {
					case 1 :
						ArrayList<Contract> newList = contractService
								.searchBySalesPerson(managerLogin.getId());
					...
					case 2 :
						InsuranceProduct selectInsuranceProduct = this
								.insuranceMenu(insuranceProductService
										.showInsuranceProductIsApproval());
						if (selectInsuranceProduct == null)
							break;
						this.registeInsuranceProduct(selectInsuranceProduct);// 보험
																				// detail이
																				// 두번
																				// 나오는
																				// 현상
	...
	private void registeInsuranceProduct(
			InsuranceProduct selectInsuranceProduct) {// 0611 수정
		try {
			System.out.println("\n---해당 보험을 가입하시겠습니까?---");
	...
	private void clientContractMenu(InsuranceProduct selectInsuranceProduct) {// 완성(21.05.26)
		switch (selectInsuranceProduct.getInsuranceProductType()) {// 보험 타입마다 다른
																	// 계약내용구현
			case ACTUALEXPENSE :
	...
	private void contractActualExpense(
		...
		try {
			...
			contract.setInsuranceExpiryDate(
					selectInsuranceProduct.getPaymentPeriod());
			contract.setSalesPerson((SalesPerson) managerLogin);
			System.out.println("고객의 보험료는 "
					+ selectInsuranceProduct.calculationRate(clientLogin)
					+ " 입니다.");
			System.out
					.println(contractService.registerInsuranceProduct(contract)
							? "계약 작성이 완료되었습니다. 계약 심사 후 보험가입 승인여부가 결정됩니다."
	...
	private void contractCancer(InsuranceProduct selectInsuranceProduct) {// 0611
																			// 수정
		try {
			Contract contract = new Contract();
			System.out.println("자신의 암 경력을 입력해주세요.");
			System.out.println(
					"1.췌장암(1.6) 2.폐암(1.5) 3.위암(1.4) 4.대장암(1.3) 5.간암(1.2) 6.기타(1.1) 7.없음(1.0)");
			int input = sc.nextInt();
			...
			System.out.println(CancerType.values()[input - 1].getCancerName()
					+ " " + rate);
			System.out.println(
					"현재 암 경력은 " + CancerType.values()[input - 1].getCancerName()
							+ " " + rate + "입니다.");
			clientLogin.getMedicalHistory()
					.setClientCancerCareer(CancerType.values()[input - 1]);
			...
			System.out.println(CancerType.values()[a - 1].getCancerName() + " "
					+ cancerRate);
			clientLogin.getMedicalHistory()
					.setFamilyCancerCareer(CancerType.values()[input - 1]);
	...
	private void contractPension(InsuranceProduct selectInsuranceProduct) {// 0611
																			// 수정
		try {
			Contract contract = new Contract();
			contract.setClient(clientLogin);
	...
	private void contractLife(InsuranceProduct selectInsuranceProduct) {// 0611
																		// 수정
		try {
			Contract contract = new Contract();
			contract.setClient(clientLogin);
	...
	private void signUpInsuranceProductMenu() {// 완성(21.05.28) 0611 수정
		while (true) {
			try {
				ArrayList<Contract> contractList = contractService
						.selectByApproval(true);
				if (contractList.size() > 0) {
					System.out.println("조회할 보험을 선택해주세요.");
					for (int i = 0; i < contractList.size(); i++)
						System.out.println((i + 1 + ". 보험상품명: ") + contractList
								.get(i).getInsuranceProduct().getProductName()
								+ " 보험종류: "
								+ contractList.get(i).getInsuranceProduct()
					...
					switch (b) {
						case 1 :
							System.out
									.println("정말로 해지하시겠습니까? 보험료는 환불이 불가능합니다.");
		...
		while (true) {
			...
			try {
				...
				if (month[a - 1]) {
					...
					month[a - 1] = true;
					// switch
					// (contract.getInsuranceProduct().getInsuranceProductType())
					// {
					// case ACTUALEXPENSE:
					// System.out.println(
					// "납부하실 보험료는 " +
					...
					// System.out
					// .println("납부하실 보험료는 " +
					// contract.getInsuranceProduct().getBasicInsurancePremium());
					// System.out.println("보험료가 납부되었습니다.");
					// break;
					// case PENSION:
					// System.out
					...
					// }
					System.out.println(
							"납부하실 보험료는 " + contract.getInsuranceProduct()
									.calculationRate(clientLogin));
				...
				if (contractList.size() > 0) {
					...
					switch (a) {
						case 1 :
							...
							applyAccident.setInsuranceProduct(
									contract.getInsuranceProduct());
							applyAccident.setReceptionDate(new Date());
							System.out.println(contractService
									.addApplyAccidentList(applyAccident)
											? "\n사고접수 신청이 완료되었습니다."
											: "\n사고접수 신청이 실패하였습니다.");
							return;
						case 2 :
		...
		while (true) {
			try {
				...
				if (!accidentList.isEmpty()) {
					...
					for (Accident accident : accidentList) {
						System.out.println((a + 1) + ". " + "상품명: "
								+ accident
										.getInsuranceProduct().getProductName()
								+ " 보험종류: "
								+ accident.getInsuranceProduct()
										.getInsuranceProductType()
								+ " 접수날짜: "
					...
					System.out.println("상품명: "
							+ acc.getInsuranceProduct().getProductName());
					System.out.println("보험종류: " + acc.getInsuranceProduct()
							.getInsuranceProductType());
					System.out.println(
							"접수날짜: " + acc.getReceptionDate().toString());
					...
					switch (c) {
						case 1 :
							System.out.println(
									contractService.deleteAccidentList(acc)
											? "사고접수 삭제가 완료되었습니다."
											: "사고접수 삭제가 실패하였습니다."); // 0531 메서드명
																	// 변경
							break;
						case 2 :
							return;
	...
	private void clientRegisterMenu() { // 0611 수정
		while (true) {
			try {
				...
				System.out.println(clientService.register(client)
						? "회원가입이 완료되었습니다."
						: "회원가입에 실패했습니다.");
	...
	private void clientDeleteMenu() { // 0611 수정
		...
		if (clientService.login(id, pw) != null) {
			...
			switch (a) {
				case 1 :
					System.out.println(clientService.delete(id, pw)
							? "삭제가 완료되었습니다."
							: "삭제에 실패하였습니다.");
	...
	private InsuranceProduct insuranceMenu(
			ArrayList<InsuranceProduct> insuranceProductList) { // 0611 수정
		try {
			System.out.println("\n---InsuranceList---");
			int i = 1;
			if (insuranceProductList.isEmpty()) {
				...
				for (InsuranceProduct insuranceProduct : insuranceProductList) {
					System.out.println(
							i + ". " + insuranceProduct.getProductName() + " "
									+ insuranceProduct.getInsuranceProductType()
											.getInsuranceName());
					i++;
				}
				System.out.println("\n보험상품의 번호를 입력해주세요.");
				InsuranceProduct selectInsurance = insuranceProductList
						.get(sc.nextInt() - 1);
	...
	private void showInsuranceProductDetail(InsuranceProduct insuranceProduct) {
		...
		System.out.println(
				"기본보험료: " + insuranceProduct.getBasicInsurancePremium());
		System.out.println("보험 종류: " + insuranceProduct
				.getInsuranceProductType().getInsuranceName());
